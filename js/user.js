...
function hideAll() {
  ...
  document.getElementById("profile-rate-download").textContent = "-";
  document.getElementById("profile-rate-upload").textContent = "-";
  document.getElementById("profile-max-sessions").textContent = "-";
  document.getElementById("profile-uptime-limit").textContent = "-";
  document.getElementById("profile-session-time").textContent = "-";
  document.getElementById("profile-validity-display").textContent = "-";
}
  ...
  ajax.post("../api/getUser", null, function (responseText) {
    try {
      ...
      if (obj.success) {
        var tableRows = "";
        tableRows +=
          "<tr><td>Username:</td><td>" + obj.data.username + "</td></tr>";
        tableRows +=
          "<tr><td>Max simultaneous sessions:</td><td>" +
          (obj.data.sharedUsers == 0 ? "unlimited" : obj.data.sharedUsers) +
          "</td></tr>";
        tableRows +=
          "<tr><td>Currently active sessions:</td><td>" +
          obj.data.activeSess +
          "</td></tr>";
        tableRows +=
          "<tr><td>Total download:</td><td>" + obj.data.download + "</td></tr>";
        tableRows +=
          "<tr><td>Total upload:</td><td>" + obj.data.upload + "</td></tr>";
        tableRows +=
          "<tr><td>Total uptime:</td><td>" + obj.data.uptime + "</td></tr>";
...
function loadProfiles() {
  ajax.post("../api/getProfiles", null, function (responseText) {
    try {
      ...
      if (obj.success) {
        var tableRows =
          "<tr><th>Name</th><th>Validity</th><th>Starts at</th></tr><tbody>";
        ...
        for (var i = 0; i < profiles.length; i++) {
          ...
          tableRows +=
            "<td>" + UM_PROF_STARTS_AT[profiles[i].startsAt] + "</td>";
          tableRows += "</tr>";
        }
        tableRows += "</tbody>";
...
function loadSessions() {
  ajax.post("../api/getUserSessions", null, function (responseText) {
    try {
      ...
      if (obj.success) {
        var tableRows =
          "<tr><th>Start Time</th><th>End Time</th><th>Uptime</th>" +
        ...
        if (pTypes.length) {
          ...
          for (var i = 0; i < pTypes.length; i++) {
            ...
            if (pTypes[i].typeId === 1) {
              resultHtml +=
                '<input style="margin:8px 15px;" type="radio" name="paymethod" value="1" ' +
                checkStr +
                ' /><img src="../img/PayPal_mark_37x23.gif" alt="PayPal" /><br />';
            } else if (pTypes[i].typeId === 2) {
              resultHtml +=
                '<input style="margin:8px 15px;" type="radio" name="paymethod" value="2" ' +
                checkStr +
                ' />Credit card <span style="font-size:12px;color:#762824;">(via Authorize.net)</span> <br />';
            }
          }

          resultHtml += "</form>";
          resultHtml +=
            '<button style="width: 100px; margin-left: 40px;margin-top: 20px;" onclick="onBuyClick(this);">Buy</button>';
        } else {
          resultHtml +=
            '<div style="text-align:center;padding:20px;">No payment method enabled.</div>';
        }
...
function loadPayments() {
  ajax.post("../api/getUserPayments", null, function (responseText) {
    try {
      ...
      if (obj.success) {
        var tableRows =
          "<tr><th>Type</th><th>Profile</th><th>Start</th>" +
        ...
        for (var i = payments.length - 1; i >= 0; i--) {
          ...
          if (
            ...
            tableRows +=
              '<td><button value="' +
              payments[i].id +
              '" onclick="onContinueBuyClick(this)">Continue</button></td>';
          } else {
            tableRows += "<td></td>";
          }
          tableRows += "</tr>";
